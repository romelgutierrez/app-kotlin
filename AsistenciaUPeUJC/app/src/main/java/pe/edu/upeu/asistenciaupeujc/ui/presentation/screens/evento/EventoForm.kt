...
fun EventoForm(
    ...
    val actividadD:Evento
    if (text!="0"){
        actividadD = Gson().fromJson(text, Evento::class.java)
    }else{
        actividadD= Evento(0,"","", "","","","","","","","", "", "", "", "", "", periodoId = "1")
    }
...
fun formulario(id:Long,
    ...
    val person=Evento(0,"","", "","","","","","","","", "", "", "", "", "", periodoId = "1")
    ...
    Scaffold(modifier = Modifier.padding(top = 60.dp, start = 16.dp, end = 16.dp, bottom = 32.dp)){
        BuildEasyForms { easyForm ->
            Column(modifier = Modifier.verticalScroll(rememberScrollState())) {
                NameTextField(easyForms = easyForm, text =evento?.nombreEvento!!,"Nomb. Evento:", MyFormKeys.NAME )
                var listE = listOf(
                    ComboModel("Activo","Activo"),
                    ComboModel("Desactivo","Desactivo"),
                )
                ComboBox(easyForm = easyForm, "Estado:", evento?.estado!!, listE)

                var listEv = listOf(
                    ComboModel("SI","SI"),
                    ComboModel("NO","NO"),
                )
                ComboBoxTwo(easyForm = easyForm, "Evaluar:", evento?.evaluar!!, listEv)


                DatePickerCustom(easyForm = easyForm, label = "Fecha", texts = evento?.fecha!!, MyFormKeys.FECHA,"yyyy-MM-dd")
                TimePickerCustom(easyForm = easyForm, label = "Hora", texts = evento?.horai!!, MyFormKeys.TIME, "HH:mm:ss")
                TimePickerCustom(easyForm = easyForm, label = "Min. Toler", texts = evento?.minToler!!, MyFormKeys.TIME_TOLER,"HH:mm:ss")
                NameTextField(easyForms = easyForm, text = evento?.mater!!, "Materiales:", MyFormKeys.MATERIALES )
                DropdownMenuCustom(easyForm = easyForm, label = "Validar Inscripcion:", evento.validInsc, list =listEv, MyFormKeys.VALIDINSCRIP )
                DropdownMenuCustom(easyForm = easyForm, label = "Validar Asis. SubActividad:", evento.asisSubact, list =listEv, MyFormKeys.ASISSUBACT )
                DropdownMenuCustom(easyForm = easyForm, label = "Reg. Entrada y Salida:", evento.entsal, list =listEv, MyFormKeys.ENTSAL )
                DropdownMenuCustom(easyForm = easyForm, label = "Reg. Offline:", evento.offlinex, list =listEv, MyFormKeys.OFFLINE )

                Row(Modifier.align(Alignment.CenterHorizontally)){
                    AccionButtonSuccess(easyForms = easyForm, "Guardar", id){
                        val lista=easyForm.formData()
                        person.nombreEvento=(lista.get(0) as EasyFormsResult.StringResult).value
